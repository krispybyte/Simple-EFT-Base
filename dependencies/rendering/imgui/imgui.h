...
struct ImVec2
{
    ...
    ImVec2 operator*( const float v ) {
        return ImVec2{ x * v, y * v };
    }

    ImVec2 operator+( float v ) {
        x = x + v;
        y = y + v;
        return *this;
    }
...
namespace ImGui
{
    ...
    IMGUI_API  void SpinnerRound(const char* label, float radius, float thickness, int num_segments, ImVec4 color);
    ...
    IMGUI_API bool          TabEx(const char* label, const ImVec2& size_arg, int selectedTab, ImGuiButtonFlags flags);
    IMGUI_API bool          Tab(const char* label, int selectedTab, const ImVec2& size = ImVec2(0, 0));      // tab
    IMGUI_API bool          TabNoLine(const char* label, int selectedTab, const ImVec2& size_arg);      // tab no line
    IMGUI_API bool          TabBG(const char* label, int selectedTab, const ImVec2& size = ImVec2(0, 0));   // tab w / bg
    ...
    IMGUI_API bool          InputText(const char* label, char* buf, size_t buf_size, ImVec2, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
...
enum ImGuiButtonFlags_
{
    ...
    ImGuiButtonFlags_NoBG = 1 << 3,
    ImGuiButtonFlags_NoLines = 1 << 4,
