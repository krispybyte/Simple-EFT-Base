...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    else
    {
        ...
        ImRect bb(text_pos + ImVec2(9, 0), text_pos + text_size);
        ItemSize(text_size, 0.0f);
        if (!ItemAdd(bb, 0))
            return;
...
ImVec2 ImGui::CenteredText(const char *text, ...)
{
    ImVec2 s = ImGui::CalcTextSize(text);

    auto pos = (ImGui::GetWindowSize( ).x - s.x) / 2;
    ImGui::SetCursorPosX(pos);
    ImGui::Text(text);

    return (ImGui::GetWindowSize( ) - s) / 2;
}
...
bool ImGui::CenteredButton(const char *text, ImVec2 size)
{
    ImVec2 s = ImGui::CalcItemSize(size, 120, 120);
...
bool ImGui::TabEx(const char* label, const ImVec2& size_arg, int selectedTab, ImGuiButtonFlags flags)
{
    ...
    static std::map<ImGuiID, float> selected_line_animation;
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = ImGui::CalcItemSize(size_arg, 27, label_size.y + style.FramePadding.y * 2.0f);
    ...
    // Render
    const ImU32 col = ImGui::GetColorU32(ImGuiCol_Accent);
    ...
    it_hover->second = ImClamp(it_hover->second + 1.6f * ImGui::GetIO().DeltaTime * (hovered ? 1.f : -1.f), 0.f, 1.f);

    ImU32 hovering_color = ImGui::GetColorU32(ImGuiCol_Accent, it_hover->second * 0.4f);

    // line animation
    auto it_hover_line = line_animation.find(id);
    if (it_hover_line == line_animation.end())
    {
        line_animation.insert({ id, {1.f} });
        it_hover_line = line_animation.find(id);
    }
    it_hover_line->second = ImClamp(it_hover->second + 2.5f * GetIO().DeltaTime * ((hovered) ? 4.f : -1.f), -1.f, 0.6f);

    // selected animation
    auto it_selected = selected_line_animation.find(id);
    if (it_selected == selected_line_animation.end())
    {
        selected_line_animation.insert({ id, {0.f} });
        it_selected = selected_line_animation.find(id);
    }

    it_selected->second = ImClamp(it_selected->second + 1.6f * ImGui::GetIO().DeltaTime * (pressed ? 1.f : -1.f), 0.f, 1.f);

    ImU32 selected_clr = ImGui::GetColorU32(ImGuiCol_Accent, it_selected->second * 0.4f);


    auto it_selected_line = line_animation.find(id);
    if (it_selected_line == line_animation.end())
    {
        line_animation.insert({ id, {1.f} });
        it_selected_line = line_animation.find(id);
    }
    it_selected_line->second = ImClamp(it_selected_line->second + 0.5f * GetIO().DeltaTime * ((pressed && held) ? 4.f : -1.f), -0.6f, 0.3f);

    //
    //if (!(flags & ImGuiButtonFlags_NoBG))
    //    ImGui::RenderFrame(bb.Min + ImVec2(30 * it_selected_line->second + 6, size_arg.y - 6), bb.Max - ImVec2(30 * it_selected_line->second + 6, 5), col, true, 0.f);  // selected


    it_hover_line->second = ImClamp(it_hover->second + 2.5f * GetIO().DeltaTime * ((hovered) ? 4.f : -1.f), -1.f, 0.6f);

    //if (!(flags & ImGuiButtonFlags_NoLines))
    //    ImGui::RenderFrame(bb.Min + ImVec2(-it_hover_line->second * 8.f, size_arg.y) + ImVec2(10, -6), bb.Max + ImVec2(it_hover_line->second * 8.f, 0) - ImVec2(10, 5), hovering_color, true, 0.f);  // hover   

    // gradient
    //ImGui::GetForegroundDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(-it_hover_line->second * 8.f, size_arg.y ) + ImVec2(10, -6), bb.Max + ImVec2(it_hover_line->second * 8.f, 0) - ImVec2(10, 35), hovering_color, hovering_color, ImGui::GetColorU32(ImGuiCol_Transparent), ImGui::GetColorU32(ImGuiCol_Transparent));  // hover   
    ...
    text_selected->second = ImClamp(text_selected->second + 0.5f * ImGui::GetIO().DeltaTime * (!(flags & ImGuiButtonFlags_NoBG) || hovered ? 7.f : -3.2f), -0.2f, 1.f);

    const ImVec4 ac = ImGui::GetStyleColorVec4(ImGuiCol_Accent);
    ...
    if (!(flags & ImGuiButtonFlags_NoLines))
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(ac.x, ac.y, ac.z, 0.2f * text_selected->second * 3));
    else
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.6f, 0.6f, 0.6f, 0.2f * text_selected->second * 3));
    ImGui::RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
    ImGui::PopStyleColor();
...
bool ImGui::Tab(const char* label, int selectedTab, const ImVec2& size_arg)
{
    return ImGui::TabEx(label, size_arg, selectedTab, ImGuiButtonFlags_NoBG);
}

bool ImGui::TabNoLine(const char* label, int selectedTab, const ImVec2& size_arg)
{
    return ImGui::TabEx(label, size_arg, selectedTab, ImGuiButtonFlags_NoLines);
}

bool ImGui::TabBG(const char* label, int selectedTab, const ImVec2& size_arg)
{
    return ImGui::TabEx(label, size_arg, selectedTab, ImGuiButtonFlags_None);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 check_col = GetColorU32(ImGuiCol_Accent, 0.2f * text_selected->second * 6);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(9, 5), check_bb.Max - ImVec2(2, 6), ImGui::GetColorU32(ImGuiCol_White, 0.06f), 2.f);
    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(9, 5), check_bb.Max - ImVec2(2, 6), check_col, 2.f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(6, 0), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText("", buf, IM_ARRAYSIZE(buf), ImVec2( 0, 0 ), flags)) // PushId(label) + "" gives us the expected ID from outside point of view
    ...
    {
        if (InputText(label, buf, IM_ARRAYSIZE(buf), ImVec2( 0, 0 ), flags))
            value_changed = DataTypeApplyOpFromText(buf, g.InputTextState.InitialTextA.Data, data_type, p_data, format);
    }
    if (value_changed)
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImVec2 size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, NULL, buf, (int)buf_size, size_arg, flags, callback, user_data);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    ImGui::GetStyle().WindowPadding = { 10.f, 4.f };
    ...
    const ImVec2 pos = window->DC.CursorPos + ImVec2(6, 0);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags, { 20, 14 }))
        {
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImGui::GetOverlayDrawList()->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha, 12);
    ImGui::GetOverlayDrawList()->AddCircle(sv_cursor_pos, sv_cursor_rad + 1, col_midgrey, 12);
    ImGui::GetOverlayDrawList()->AddCircle(sv_cursor_pos, sv_cursor_rad, col_white, 12);
    ...
    if (alpha_bar)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min, bar1_bb.Max, 0, bar1_bb.GetWidth() / 4.0f, ImVec2(0.0f, 0.0f));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    else
    {
        ...
        if (col_rgb_without_alpha.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_rgb_without_alpha), grid_step, ImVec2(off, off), 3.f);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_rgb_without_alpha), 3.f);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
...
bool ImGui::hotkey( const char* label, int* k ) {
    ...
    const ImVec2 label_size = CalcTextSize(buf_display.c_str( ), NULL, true);
    ...
    static auto last_press = GetTickCount64( );
    ...
    if (g.ActiveId == id ) {
        if (!value_changed) {
            for (auto i = 0x1; i <= 0x7B; i++) {
                if ( GetTickCount64( ) - last_press > 300 ) {
    ...
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, ImGui::GetColorU32(ImVec4(0.20f, 0.25f, 0.30f, 1.0f)), true, style.FrameRounding);
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, buf_display.c_str( ), NULL, NULL, style.ButtonTextAlign, &bb );
